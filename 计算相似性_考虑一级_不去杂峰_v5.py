...
# 划分并行任务区间
def split_mission(dot_id_list, dot_id_MS1_dict, dot_id_MS2_dict, dot_id_mass_dict, data_save_dir, mission_num, sim_threshold):
    dot_id_list.sort()
    ...
    for i in range(mission_num):
        ...
        split_mission_list.append((start_index, end_index, dot_id_list, dot_id_MS1_dict, dot_id_MS2_dict, dot_id_mass_dict, data_save_dir, sim_threshold))
    return split_mission_list

# 并行计算所有分子间的相似性
def get_all_sim(start_end):
    writen_list = []
    start, end, dot_id_list, dot_id_MS1_dict, dot_id_MS2_dict, dot_id_mass_dict, data_save_dir, sim_threshold = start_end
    dot_id_list_0 = dot_id_list[start:end]
    for dot_id0 in tqdm(dot_id_list_0):
        ms0 = dot_id_MS1_dict[dot_id0]
        msms0 = dot_id_MS2_dict[dot_id0]
        mass0 = dot_id_mass_dict[dot_id0]
        for dot_id1 in dot_id_list:
            if dot_id0 != dot_id1:
                ms1 = dot_id_MS1_dict[dot_id1]
                msms1 = dot_id_MS2_dict[dot_id1]
                mass1 = dot_id_mass_dict[dot_id1]
                if abs(mass0-mass1) < 100:  # 141是结构差模型的最大质量差
                    if msms0 != None and msms1 != None:
                        sim1 = MS_match_type1(ms0, ms1)         # 一级匹配率
                        sim2 = MS_match_type1(msms0, msms1)     # 二级匹配率
                        sim3 = MS_sim(msms0, msms1)             # 二级相似性
                        sim = 0.2*sim1 + 0.2*sim2 + 0.6*sim3
                    else:
                        sim = 0
...
def main(data_save_dir, cpu_num, sim_threshold):
    ...
    for i in range(2, ws.max_row+1):
        ...
        if msms_str != None:
            dot_id_MS1_dict[ws.cell(i,1).value] = ws.cell(i,6).value
    ...
    split_mission_list = split_mission(dot_id_list, dot_id_MS1_dict, dot_id_MS2_dict, dot_id_mass_dict, data_save_dir, cpu_num, sim_threshold)
